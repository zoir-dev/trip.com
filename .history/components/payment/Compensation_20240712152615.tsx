...
interface ComponsationListType {
    addedBag: string
    liveChat: string
    processedChat: string
    Access: string
}

const ComponsationList:ComponsationListType[] = [
    {
        ...
        processedChat: "AirHelp's specialists handle the entire process of getting your compensation",
        Access: "Access to a VIP lounge if your flight is delayed by more than 1 hour or canceled less than 6 hours before departure."
    }
]
...
const Compensation = () => {
  return (
    <Card className='mt-8 border-none'>
        ...
        <CardContent>
           <div>
            {ComponsationsList.map((item, index) => (
                <div key={index}>
                    <p className='text-[#0f294d] text-[14px]'>{item.addedBag}</p>
                    <p className='text-[#8592a6] text-[14px]'>{item.liveChat}</p>
                    <p className='text-[#8592a6] text-[14px]'>{item.processedChat}</p>
                    <p className='text-[#8592a6] text-[14px]'>{item.Access}</p>
                </div>
            ))}
           </div>
        </CardContent>
